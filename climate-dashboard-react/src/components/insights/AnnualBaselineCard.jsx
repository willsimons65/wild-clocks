...
import { classifyBaselineRange } from "@/data/insights/classifyBaselineRange";
...
const ANNUAL_BASELINE_INFO = (
  <>
    <p className="mb-2">
      ...
      wet or dry a place is. It compares how much rain falls in a year with how much moisture the
      atmosphere would remove. Values below 1 indicate water-limited environments; values above 1
      indicate persistent moisture surplus.
    </p>
  </>
);
...
export default function AnnualBaselineCard({ rainfallData }) {
  ...
  const classification = result.hasData
    ? classifyBaselineRange(result.min, result.max)
    : null;

  const Header = (
    <div className="flex items-start justify-between">
      <h2 className="text-sm font-medium opacity-80">
        Annual baseline index
      </h2>
