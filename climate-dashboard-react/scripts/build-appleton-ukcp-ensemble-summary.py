...
def main() -> None:
    ...
    input_files = sorted(
        ...
        for path in args.input_directory.glob(
            "appleton-heat-stress-member-*.csv"
        )
        if re.fullmatch(
            r"appleton-heat-stress-member-\d{2}-\d{4}-\d{4}\.csv",
            path.name,
        )
    )
    ...
    if not input_files:
        raise FileNotFoundError(
            "No annual Appleton heat-stress member CSVs found in "
    ...
    for first_year, period_members in member_summary.groupby(
        ...
        member_output = (
            ...
            / f"appleton-heat-stress-member-summary-{period}.csv"
        )

        ensemble_output = (
            args.output_directory
            / f"appleton-heat-stress-ensemble-summary-{period}.csv"
        )
